...
This is where all the metadata and code documentation will go. 

# Resources
Softlinks to original data go here 

# Workflow/Scripts
- All the code goes here. Customize as needed & update the README.md file
- Can also include a ```sandbox/``` folder to temporarily store code that you no longer need.
  - I have a function that outputs the min and max of each variable. As this is no longer needed for the       project, I will store it in the ```sandbox``` for potential future use.

# Results
All tables,figures, model results etc  go here